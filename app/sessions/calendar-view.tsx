...
export async function CalendarView({
  ...
  const m = month && /^\d{4}-\d{2}$/.test(month) ? month : thisMonth();
  ...
  const selectedRows = sel ? byDate[sel] ?? [] : [];
    ...
    <>
      <header className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          <Link
            prefetch
            href={`/sessions?v=calendar&m=${shiftMonth(m, -1)}`}
          ...
          <Link
            prefetch
            href={`/sessions?v=calendar&m=${thisMonth()}`}
        ...
        {cells.map((c, i) => {
          ...
          return (
            <Link
              key={i}
              prefetch
              href={`/sessions?v=calendar&m=${m}&d=${c.date}`}
      ...
      {sel && (
        <section className="mt-6">
          <h2 className="text-[10px] uppercase tracking-[0.25em] text-[#888892] mb-3 font-mono">
            {sel} · {selectedRows.length} session{selectedRows.length === 1 ? '' : 's'}
          </h2>
          {selectedRows.length > 0 ? (
            <ul className="space-y-2">
              {(selectedRows as unknown as Array<
                Session & {
                  type:
            ...
            <p className="text-[11px] uppercase tracking-[0.2em] text-[#5a5a62]">
              ...
              <Link prefetch href="/sessions?v=list" className="text-[#a3e635] underline">
                Log one →
              </Link>
            </p>
          )}
      ...
      <div className="mt-6 flex flex-wrap gap-3 text-[10px] uppercase tracking-[0.15em]">
